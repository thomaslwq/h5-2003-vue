  ...
  <div class="glee-login">
    ...
    <div class="glee-login-middle">
      <div class="login-middle-user">
        ...
        <input type="text" id="name-notice" v-model="username" @keyup="lossName()" />
        <span ref="user-notice" :class="['user-notice',{'right':right=='right'},{'error':error=='error'}]"></span>
      </div>
      <div class="login-middle-pass">
        <label for="pass-notice">密码 *</label>
        <input type="password" id="pass-notice" v-model="userpass" @keyup="lossPass()" />
        <span ref="pass-notice" :class="['pass-notice',{'right':passright=='right'},{'error':passerror=='error'}]"></span>
      </div>
      <div class="login-middle-btn" @click="loginHandler">登录</div>
...
export default {
  ...
  methods: {
    loginHandler() {
      ...
      this.$axios
        ...
        .then((res) => {
          if (res.code == 200) {
            this.$router.push("/");
            localStorage.setItem("userID", res.results[0].userID);
          } else {
            this.$notify.error({
              title:"错误",
              message:"账号或者密码不正确",
    ...
    lossName() {
      ...
      if (!(reg1.test(email) || reg2.test(email))) {
        this.error = "error"
        this.right = ""
        this.$refs["user-notice"].innerText = "账号必须要使用字母开头，且普通账号长度不能超过15位"
      } else {
        this.right = "right";
        this.error = ""
        this.$refs["user-notice"].innerText = "输入规则正确"
      }
    },
    lossPass() {
      ...
      let password = this.userpass;
      // 验证密码
      let reg3 = /^[a-zA-Z0-9]{6,15}$/;
      if (!reg3.test(password)) {
        this.passerror = "error"
        this.passright = ""
        this.$refs["pass-notice"].innerText = "密码长度至少为六位数，且不超过15位"
      } else {
        this.passright = "right";
        this.passerror = ""
        this.$refs["pass-notice"].innerText = "输入规则正确"
      }
    },
    toRegister() {
  ...
  // beforeMount() {console.log(window.scrollTop)}, //生命周期 - 挂载之前
